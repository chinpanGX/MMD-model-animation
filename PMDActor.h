...
public:
	...
	~PMDActor() {}
	PMDActor* Clone() {} // クローンは頂点およびマテリアルは共通のバッファを見るようにする
	void Update();
	void Draw();
private:
	HRESULT CreateMaterialData();			// 読み込んだマテリアルをもとにマテリアルバッファを作成
	HRESULT CreateMaterialAndTextureView(); // マテリアル＆テクスチャのビューを作成
	HRESULT CreateTransformView();			// 座標変換用ビューの生成
