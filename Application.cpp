...
bool Application::Init()
{
	...
	//DirectX12ラッパー生成＆初期化
	_dx12.reset(new DX12Wrapper(m_hWnd));
...
void Application::Run()
{
	...
	while (true) {
		...
		//PMD用の描画パイプラインに合わせる
		_dx12->GetCommandList()->SetPipelineState(_pmdRenderer->GetPipelineState());
		//ルートシグネチャもPMD用に合わせる
		_dx12->GetCommandList()->SetGraphicsRootSignature(_pmdRenderer->GetRootSignature());

		_dx12->GetCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		...
		//フリップ
		_dx12->GetSwapchain()->Present(1, 0);
	}
}
